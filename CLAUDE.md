...
- **GASのデプロイは clasp で自動化済み（2026-06-11〜）**：ローカルで `.gs` を編集→`clasp push --force && clasp deploy --deploymentId <既存ID>` でWeb App URL不変のまま本番反映。対象3プロジェクト（social-auto-post／onboarding-diagnosis／reme-post）の deploymentId・ワンライナーは memory `reference_clasp_setup.md` 参照。GASエディタを開いてコピペする必要なし
